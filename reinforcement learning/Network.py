...
LastEditTime: 2022-07-24 06:45:25
FilePath: /matching/reinforcement learning/Network.py
    ...
    def test(self):
        ...
        for i in tqdm(range(len(self.Nodes))):
            for j in range(len(self.Nodes)):
                O.append(self.Nodes[i])
                D.append(self.Nodes[j])
                distance.append(self.get_path(self.Nodes[i].id, self.Nodes[j].id))
