...
LastEditTime: 2022-07-05 09:57:50
FilePath: /matching/reinforcement learning/Critic.py
...
class Critic:
    ...
    def update(self, memory):
        ...
        values = self.value_net(state_batch).squeeze(-1) # 计算当前状态(s_t)对应的V(s_t)
        next_values = self.target_net(next_state_batch).squeeze(-1) # 计算下一时刻的状态值
        # 计算期望的Q值，对于终止状态，此时done_batch[0]=1, 对应的expected_q_value等于reward
        expected_values = reward_batch + self.gamma * next_values #  * (1-done_batch)
        loss = nn.MSELoss()(values, expected_values)  # 计算均方根损失
        # 优化更新模型
