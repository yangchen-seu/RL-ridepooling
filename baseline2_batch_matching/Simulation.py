...
class Simulation():
    ...
    def batch_matching(self, takers, vehicles, seekers):
        ...
        # print('len(vehicles)',len(vehicles),'len(takers)', len(takers),'len(seekers)',len(seekers))
        ...
        for taker in takers:
            if taker.reposition_target == 1:
                if self.time - taker.order_list[0].begin_time_stamp - \
                    ...
                    self.saved_travel_distance += travel_distance
        ...
        # print('派送用时{},takers{},vehicles{}'.format(
        #     end-start, len(takers), len(vehicles)))
